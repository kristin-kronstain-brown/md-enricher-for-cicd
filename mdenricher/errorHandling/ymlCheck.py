...
def ymlCheck(self, details, ISSUE_WARNINGS, yml_files_list, yml_files_list_folderAndFile, location_dir, location_name):
    ...
    if not yml_files_list == [] and self.location_tag_processing == 'on':
        # Conditional self.log
        ...
        for ymlFile in yml_files_list:
            ...
            if os.path.isfile(path + ymlFile):
                try:
                    ...
                    with open(path + ymlFile, 'r', encoding="utf8", errors="ignore") as ymlFileVar:
                        try:
                            ...
                            addToErrors('YML not formatted properly. Check ' + ymlFile + ' for errors. ' +
                                        str(exc), folderAndFile, '', details, self.log, build, '', '')
                        else:
                            self.log.debug('YML validated: ' + ymlFile)
                            ...
                            if (ymlFile == '/toc.yaml') and (details["ibm_cloud_docs"] is True):
                                if os.path.isfile(details["workspace"] + '/toc_schema.json'):
                                    self.log.debug('IBM Cloud toc_schema.json exists. toc.yaml can be verified.')
                                    with open(details["workspace"] + '/toc_schema.json', "r", encoding="utf8", errors="ignore") as stream:
                                        try:
                                            config_schema = yaml.safe_load(stream)
                                            self.log.debug(str(config_schema)[0:500])
                                        except yaml.YAMLError as exc:
                                            self.log.warning(exc)

                                        try:
                                            # Remove if when schema supports valid tagging in it.
                                            if ('flag: ' not in str(config_schema) and
                                                    'tag: ' not in str(config_schema) and
                                                    'x-visible-environment: ' not in str(config_schema)):
                                            ...
                                            addToErrors('YML not formatted properly. Check the toc.yaml for tagging errors. ' +
                                                        str(error), folderAndFile, '', details, self.log, build, '', '')
                                else:
                                    self.log.debug('toc_schema.json was not found. The toc.yaml cannot be verified.')
                    ...
                    addToErrors('YML not formatted properly. ' + error, folderAndFile, '', details, self.log, build, '', '')
